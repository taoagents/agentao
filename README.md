...
#### Run
Run the validator script via `run_validator.sh`:
...
./scripts/run_validator.sh -- \
    ...
    --wallet.hotkey <hotkey>
```
Arguments before the `--` get passed to the `pm2 start` command, and arguments after get passed to the `python neurons/validator.py` command. If running in a virtual environment, add a `--interpreter <venv>/bin/python3` argument before the `--`.
